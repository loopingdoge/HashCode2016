...
Il problema affrontato in questo progetto porta il nome di **Delivery**, e corrisponde al quesito posto da Google per l'HashCode 2016. Gli Hash Code sono competizioni dedicate alla programmazione che sfidano team di sviluppatori presenti in Europa, Medio Oriente e Africa a risolvere un problema concreto tra quelli affrontati dall'azienda più influente del mondo informatico.
...
E' qui che trovano spazio i droni, veicoli elettrici che, in autonomia possono spostarsi in volo evitando il traffico e di fatto riducendo le tempistiche per gli spostamenti di merci. Allora, data una flotta di droni, una lista di ordini e di disponibilità nei depositi, come si possono pianificare le azioni da comunicare ai veivoli per completare le consegne nel modo più efficiente?
...
Non partecipando effettivamente al contest, non siamo stati sottoposti agli stretti vincoli temporali per sottoporre la nostra soluzione. Per questo abbiamo deciso di rendere il problema più realistico, non considerando la clausola che vedeva non necessario il completamento di tutti gli ordini dati in input, soddisfacendo tutti i clienti (i punteggi saranno calcolati comunque tenendo conto del limite di turni).
...
Per la risoluzione del problema, si è selezionato un approccio misto che vede alla base un sistema di **pianificazione automatica**, tecnica largamente utilizzata in ambito decisionale riguardante i trasporti.
...
- il peso corrente caricato da ogni drone,
...
Lo **stato finale**, o di goal, corrisponderà alla congiunzione delle formule che vedono i vari oggetti (quelli che fanno parte di ordini) posizionati nelle abitazioni dei clienti. Questa corrisponde ad una descrizione parziale di uno stato in cui il goal è soddisfatto.
...
La **quarta strategia** introduce un elemento aleatorio alla scelta dell'azione potenziale da aggiungere al piano. Per costruzione, il pianificatore non aggiunge mai al piano azioni di *deliver* fino a che ci sia la possibilità di effettuare una *load*, ora, la probabilità che si provi ad aggiungere una *deliver*, è la stessa della *load*. L'implementazione della strategia è presente nel file "random_action_planner.pl".
...
Le varie sessioni verranno effettuate su una mappa 50x50, dove saranno dislocati 5 depositi. Il numero di oggetti per ordine varia da 1 a 3, quindi gli oggetti da consegnare variano tra il numero di ordini e il numero di ordini moltiplicato per 3.

Per le prime tre sessioni le tipologie di prodotti saranno 5.
...
- TODO Commento risultati:


- "backtrack_stupid_planner", "stupid_planner" e "stupid_drones_planner" risentono gravemente dell'aumento del numero di droni, avere più droni aumenta le combinazioni che le strategie citate proveranno ad unificare, perdendo troppo tempo.
- l'elemento randomico che modificano "planner" facendolo diventare "random_action_planner", ha in generale effetti molto positivi sia per quanto riguarda i tempi (dimezzati) e i punteggi (duplicati).
- "drones_planner" contiene algoritmi pesanti, il cui costo crese linearmente, ma che aggiungono al piano azioni molto efficienti con grande impatto sul punteggio.
...
Stuart J. Russel e Peter Norving, Artificial Intelligence, Pearson, 2017, ISBN978-93-325-4351-5.
